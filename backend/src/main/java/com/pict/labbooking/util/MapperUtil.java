...
<<<<<<< HEAD
                ...
                .overriddenByEventId(slot.getOverriddenByEvent() != null
                        ? slot.getOverriddenByEvent().getId() : null)
                .overriddenByEventTitle(slot.getOverriddenByEvent() != null
                        ? slot.getOverriddenByEvent().getTitle() : null)
                .build();
    }

    public OverrideEventResponse toOverrideEventResponse(OverrideEvent event) {
        return toOverrideEventResponse(event, 0, 0);
    }

    public OverrideEventResponse toOverrideEventResponse(OverrideEvent event,
                                                          int bookingsOverridden,
                                                          int slotsOverridden) {
        if (event == null) return null;
        return OverrideEventResponse.builder()
                .id(event.getId())
                .title(event.getTitle())
                .description(event.getDescription())
                .type(event.getType())
                .overrideDate(event.getOverrideDate())
                .startTime(event.getStartTime())
                .endTime(event.getEndTime())
                .affectedLabs(event.getAffectedLabs().stream()
                        .map(this::toLabResponse).collect(Collectors.toList()))
                .affectedDepartments(event.getAffectedDepartments())
                .affectedDivisions(event.getAffectedDivisions())
                .createdBy(toUserResponse(event.getCreatedBy()))
                .priorityLevel(event.getPriorityLevel())
                .isMandatory(event.getIsMandatory())
